...
@Autonomous(name = "FoOAutoRedFarRev", group = "Force of Oros")
public class FoOAutoRedFarRev extends LinearOpMode
{
    /* Declare OpMode members. */
    FoOHardwareREV robot   = new FoOHardwareREV();   // Use a Pushbot's hardware
    private ElapsedTime runtime = new ElapsedTime();

    static final double     COUNTS_PER_MOTOR_REV    = 1120 ;    // Using andymark neverest 40s
    ...
    public void runOpMode() {
        ...
        while(glypos == "not visible" && runtime.seconds() <= 4 && opModeIsActive())
        {
            ...
            if (runtime.seconds() >= 1.2) //if it doesn't read anything after x seconds, then just move one and assume right (b/c its the closest)
            {
                glypos = "CENTER";
            }
        }
        ...
        encoderDrive(1.3,1.3, 0 ,3,DRIVE_SPEED);
        ...
        */
        ...
        if(glypos == "LEFT")
        {
            encoderDrive(.5, .5,0, 3, DRIVE_SPEED);
            telemetry.addData("CenterCounts", robot.center.getCurrentPosition() );
            telemetry.update();
            sleep(1000);

            encoderDrive(0,0,-1,3, .7);
            //turnDegrees(40, TURN_SPEED, 5);
            turnDegrees(35,TURN_SPEED,3);

            telemetry.addData("Centercalc2", robot.center.getCurrentPosition() );
            telemetry.update();

            sleep(1000);
            encoderDrive(3.3,3.3,0,3, DRIVE_SPEED);
        }
        else if(glypos == "CENTER")
        {
            encoderDrive(.5, .5,0, 3, DRIVE_SPEED);
            encoderDrive(0,0,-.5,3, DRIVE_SPEED);
            //turnDegrees(34, TURN_SPEED, 5);
            turnDegrees(29, TURN_SPEED,3);
            encoderDrive(2.7,2.7,0,3, DRIVE_SPEED);

        }
        else if(glypos == "RIGHT")
        {
            encoderDrive(.5, .5,0, 3, DRIVE_SPEED);
            encoderDrive(0,0,-.75,3, .7);
            turnDegrees(8, TURN_SPEED, 5);
            encoderDrive(2.25,2.25,0,3, DRIVE_SPEED);
        ...
        */
        ...
        sleep(100);
        encoderDrive(-1.0,-1.0, 0,3, DRIVE_SPEED);
    ...
    */
    ...
    public void encoderDrive(double leftInches, double rightInches,double sideInches, double timeoutS, double Speed)
    {
        ...
        if (opModeIsActive()) {
            ...
            telemetry.addData("Centercalc", sideInches * COUNTS_PER_INCH );
            telemetry.update();
            ...
            telemetry.addData("Centercalc2", centerTarget );
            telemetry.update();
