 ...
 */
@Disabled
@Autonomous(name = "FoOAutoBlueClose3Rev", group = "Force of Oros")
public class FoOAutoBlueClose3Rev extends LinearOpMode
{
    ...
    public void runOpMode() {
        ...
        */
        ...
        //RAMMUS FORMATION
        robot.vaServoRightB.setPosition(.05);
        robot.vaServoLeftB.setPosition(.94);
        robot.vaServoRightT.setPosition(.05);
        robot.vaServoLeftT.setPosition(.94);

        //get a bit closer
        encoderDrive(2.2,2.2,0,5,DRIVE_SPEED);
        telemetry.addData("STop 1", "hi");
        telemetry.update();

        //get current encoder values for calculation later
        lefttrack = robot.left.getCurrentPosition();
        righttrack = robot.right.getCurrentPosition();

        telemetry.addData("STop 2", "hi");
        telemetry.update();

        //drive foward in raming until we detect a glyph with touch sensor
        robot.left.setPower(-.8);
        robot.right.setPower(-.8);

        runtime.reset();
        double tempTime = runtime.seconds();
        while(robot.glyphsen.getState() && tempTime < .65)    //true means its not pressed (i think), does this up to 3 seconds so
        {                                                   // that it doesn't go to the other side if it doesn't detect a glyph
           telemetry.addData("Ramming", "Cubes have no rights");
           telemetry.addData("STop 3", "hi");
           telemetry.update();
           tempTime = runtime.seconds();
        }
        robot.left.setPower(0);
        robot.right.setPower(0);
        sleep(200);
        ...
        robot.verticalarm.setPower(0);
        sleep(100);
        telemetry.addData("STop 4", "hi");
        telemetry.update();


        //amount of counts moved while getting the block
        lefttrack = robot.left.getCurrentPosition() - lefttrack;
        righttrack = robot.right.getCurrentPosition() - righttrack;

        telemetry.addData("STop 4.5", "hi");
        telemetry.addData("lefttrack:", lefttrack);
        telemetry.addData("righttrack:", righttrack);
        telemetry.update();

        //count to inches conversion;
        lefttarget = (lefttrack*WHEEL_DIAMETER_INCHES*3.1415)/(COUNTS_PER_MOTOR_REV*DRIVE_GEAR_REDUCTION); //mmmmm mom's spaghetti...
        righttarget = (righttrack*WHEEL_DIAMETER_INCHES*3.1415)/(COUNTS_PER_MOTOR_REV*DRIVE_GEAR_REDUCTION);

        telemetry.addData("STop 4.8", "hi");
        telemetry.addData("lefttarget:", lefttarget);
        telemetry.addData("righttarget:", righttarget);
        telemetry.update();
        //Drive back to original position
        encoderDrive(lefttarget, righttarget, 0, 3, DRIVE_SPEED);
        sleep(100);

        encoderDrive(-1.5,-1.5,0,3,DRIVE_SPEED);

        turnDegrees(90, .1, 5); //so apparently this works... gotta doublecheck
        sleep(100);

        telemetry.addData("STop 5", "hi");
        telemetry.update();
        ...
        telemetry.addData("STop 6", "hi");
        telemetry.update();

        //put block in right height
        robot.verticalarm.setPower(1);
        sleep(600);
        robot.verticalarm.setPower(0);
        sleep(100);
        ...
        //Score the new glyph
        encoderDrive(3,3,0,3,DRIVE_SPEED);
        sleep(100);
        robot.vaServoRightB.setPosition(0);
        robot.vaServoLeftB.setPosition(1);
        robot.vaServoRightT.setPosition(0);
