...
@Autonomous(name = "FoOAutonomousWTurnRedOP2", group = "Force of Haskins")
@Disabled
public class FoOAutoWTurnREDOption2 extends LinearOpMode
{
    ...
    public void runOpMode() {
        ...
        if(blueValue > redValue) // if blue then rotate backwards to hit blue
        {
            ...
            turnDegrees(90, TURN_SPEED, 5);
            sleep(100);
            robot.touchArm.setPosition(0); //go ahead and bring out the arm just in case
            sleep(100);
            //encoderDrive(0, 0, -12, 10, 1); //left lateral movement is negative, i think

            encoderDrive(6, 6, 0, 10, 1); //get off the ramp
            sleep(100);

            turnToNorth(); //straighten the robot out
            sleep(100);

            turnDegrees(-90, TURN_SPEED, 6.); //turn 90 degrees to the right
            sleep(100);

            robot.touchArm.setPosition(0); //lower the touch arm
            sleep(100);

            encoderDrive(2.5, 2.5, 0, 10, DRIVE_SPEED); //move forward to bring it in range
            sleep(100);
        ...
        else if(redValue > blueValue)
        {
             ...
             */
            encoderDrive(4, 4, 0, 5, .2); //hit the jewel and allow the servo the raise up
            sleep(100);

            robot.colorServo.setPosition(1); //bring the colorServo back in
            sleep(500);
        ...
        sleep(500);
        encoderDrive(3, 3, 0, 5.0, DRIVE_SPEED);
