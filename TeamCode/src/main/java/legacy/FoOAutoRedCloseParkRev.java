...
@Autonomous(name = "FoOAutoRedCloseParkRev", group = "Force of Oros")
public class FoOAutoRedCloseParkRev extends LinearOpMode
{
    ...
    public void runOpMode() {
        ...
        robot.colorServoRS.setPosition(postrack); //used to be .8 before they geared up the servo
        ...
        for(int i = 0; i < 5; i++) //detecting jewel colors
        {
            ...
            postrack -= .005;
            robot.colorServoRS.setPosition(postrack);
        ...
        if(blueValue > redValue) // if blue then rotate backwards to hit blue
        {
            ...
            sleep(100);
            robot.colorServoRS.setPosition(1);
            sleep(300);
        ...
        */
        ...
        while(robot.glyphsen.getState() && tempTime < 3)    //true means its not pressed (i think), does this up to 3 seconds so
        ...
        encoderDrive(-lefttarget,-righttarget, -0, 10, DRIVE_SPEED);
        sleep(100);

        //Turn back to original position
        turnDegrees(-90, .4, 5); //so apparently this works... gotta doublecheck
        sleep(100);

        //Score the new glyph
        encoderDrive(1.5,1.5,0,5,DRIVE_SPEED);
        sleep(100);
        robot.vaServoRightB.setPosition(0);
        robot.vaServoLeftB.setPosition(1);
        robot.vaServoRightT.setPosition(0);
