 ...
 */
@Disabled
@Autonomous(name = "FoOAutoBlueFar", group = "Force of Oros")
public class FoOAutoBlueFar extends LinearOpMode
{
    /* Declare OpMode members. */
    FoOHardware robot   = new FoOHardware();   // Use a Pushbot's hardware
    ...
    public void runOpMode() {
        ...
        else if(redValue < blueValue)
        {
             ...
             */
            updateAngles();
            turnDegrees(20, TURN_SPEED, 5);
            sleep(100);
            robot.colorServo.setPosition(0);
            sleep(100);
            turnDegrees(-70, TURN_SPEED, 5);
            sleep(100);
        }
        robot.verticalarm.setPower(1);
        sleep(700);
        ...
        while(glypos == "not visible" && runtime.seconds() <= 4 && opModeIsActive())
        {
            ...
            if (runtime.seconds() >= 2) //if it doesn't read anything after 3 seconds, then just move one and assume right (b/c its the closest)
            {
                glypos = "CENTER";
            }
        }
        ...
        sleep(100);
        robot.verticalarm.setPower(0);
        turnDegrees(12, TURN_SPEED, 5);
        sleep(100);
        turnDegrees(180, TURN_SPEED, 5);



        //drive off the platform then straighten outselves out
        encoderDrive(2,2,5,DRIVE_SPEED);

        robot.blMotor.setPower(-.1);
        robot.brMotor.setPower(-.1);
        robot.flMotor.setPower(-.1);
        robot.frMotor.setPower(-.1);
        while(readAngle("y") >= 1.125 )
        {
            telemetry.addData("Z", readAngle("z"));
            telemetry.addData("y", readAngle("y"));
            telemetry.addData("x", readAngle("x"));
        ...
        */
        ...
        if(glypos == "RIGHT")
        {
            encoderDrive(.5, .5, 5, DRIVE_SPEED);
            turnDegrees(-7, TURN_SPEED, 5);
            encoderDrive(2.75,2.75,5, DRIVE_SPEED);
        }
        else if(glypos == "CENTER")
        {
            encoderDrive(.5, .5, 10, DRIVE_SPEED);
            turnDegrees(-34, TURN_SPEED, 5);
            encoderDrive(3.5,3.5,5, DRIVE_SPEED);

        }
        else if(glypos == "LEFT")
        {
            encoderDrive(.3, .3, 10, DRIVE_SPEED);
            turnDegrees(-47, TURN_SPEED, 5);
            encoderDrive(5,5,5, DRIVE_SPEED);
        ...
        */
        ...
        robot.vaServoLeftB.setPosition(.8);
        robot.vaServoRightT.setPosition(0);
        robot.vaServoLeftT.setPosition(1);

        sleep(100);
        encoderDrive(-2.00,-2.0,5, DRIVE_SPEED);
    ...
    */
    ...
    {
        //Write code to correct to a target position
        runtime.reset();
    ...
    public void encoderDrive(double leftInches, double rightInches, double timeoutS, double Speed)
    {
        ...
        if (opModeIsActive()) {
            ...
            flMotorTarget = robot.flMotor.getCurrentPosition() + (int)(leftInches * COUNTS_PER_INCH);
            blMotorTarget = robot.blMotor.getCurrentPosition() + (int)(leftInches * COUNTS_PER_INCH);
            frMotorTarget = robot.frMotor.getCurrentPosition() + (int)(rightInches * COUNTS_PER_INCH);
            brMotorTarget = robot.brMotor.getCurrentPosition() + (int)(rightInches * COUNTS_PER_INCH);
            robot.flMotor.setTargetPosition(flMotorTarget);
            robot.blMotor.setTargetPosition(blMotorTarget);
            robot.frMotor.setTargetPosition(frMotorTarget);
            robot.brMotor.setTargetPosition(brMotorTarget);
            ...
            while (opModeIsActive() &&
                    ...
                    (robot.frMotor.isBusy() && robot.flMotor.isBusy() && robot.blMotor.isBusy() && robot.brMotor.isBusy()))
            {

                // Display it for the driver.
                telemetry.addData("Path1",  "Running to %7d :%7d :%7d :%7d", blMotorTarget,  brMotorTarget, frMotorTarget, flMotorTarget);
                telemetry.addData("Path2",  "Running at %7d :%7d", robot.blMotor.getCurrentPosition(), robot.brMotor.getCurrentPosition(), robot.flMotor.getCurrentPosition(), robot.frMotor.getCurrentPosition());
                telemetry.update();
            }
